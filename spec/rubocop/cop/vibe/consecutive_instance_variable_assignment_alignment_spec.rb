...
RSpec.describe RuboCop::Cop::Vibe::ConsecutiveInstanceVariableAssignmentAlignment, :config do
  describe "#on_def" do
    context "when consecutive assignments are not aligned" do
      let(:offense_code) do
        <<~RUBY
          def setup
            @user = create(:user)
            ^^^^^ Align consecutive instance variable assignments at the = operator.
  ...
  describe "edge cases" do
    ...
    context "with mixed local and instance variable assignments" do
      it "local variable breaks the group so no alignment needed" do
        expect_no_offenses(<<~RUBY)
          def setup
            @user = create(:user)
            local = value
      ...
      it "aligns consecutive instance variables around local variable" do
        expect_offense(<<~RUBY)
          def setup
            @a = 1
            ^^ Align consecutive instance variable assignments at the = operator.
    ...
    context "with the user's example from a controller" do
      let(:offense_code) do
        <<~RUBY
          def new
    ...
    context "with multiple controller actions" do
      let(:offense_code) do
        <<~RUBY
          def new
            @subcategories = budget.subcategories
      ...
      it "registers offenses in each method" do
        expect_offense(offense_code)
      end

      it "autocorrects each method independently" do
        expect_offense(offense_code)
